...
export default function CleanHome() {
  ...
  return <main className="clean-site">
    <header className="clean-header"><a className="clean-brand" href="/"><i>光</i><span><b>{copy.siteName}</b><small>{copy.siteCode}</small></span></a><nav><a href="#notes">{copy.navNotes}</a><a href="#topics">{copy.navTopics}</a><a href="/admin">{copy.navAdmin}</a></nav><div className="clean-final"><i /> FINAL EDITION</div></header>
    ...
    <section className="clean-featured"><div className="clean-heading"><div><span>01</span><h2>{copy.cFeaturedTitle}</h2></div><p>CURATED NOTES</p></div><div className="clean-feature-grid">{featured.map((note, index) => <button key={`${note.id}-${index}`} style={{ "--delay": `${index * 70}ms` } as React.CSSProperties} className={index === 0 ? "primary" : ""} onClick={() => setSelected(note)}><div className="note-card-meta"><small>{note.category}</small><time>{note.publishedAt} · {readMinutes(note.content)} 分钟</time></div><h3>{note.title}</h3><p>{note.summary}</p><span>阅读笔记 <i>↗</i></span></button>)}</div></section>

    <section className="clean-layout" id="notes"><div className="clean-notes"><div className="clean-heading"><div><span>02</span><h2>{copy.cRecentTitle}</h2></div><p>{filtered.length} NOTES</p></div><div className="clean-list">{filtered.map((note, index) => <button key={`${note.id}-${index}`} style={{ "--delay": `${Math.min(index, 8) * 45}ms` } as React.CSSProperties} onClick={() => setSelected(note)}><span className="list-index">{String(index + 1).padStart(2, "0")}</span><time>{note.publishedAt.slice(5).replace("-", ".")}</time><div><small>{note.category} · 预计 {readMinutes(note.content)} 分钟</small><h3>{note.title}</h3><p>{note.summary}</p></div><span className="list-arrow">↗</span></button>)}</div></div>
    ...
    {selected && <div className="clean-reader-bg" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}><article className="clean-reader"><button onClick={() => setSelected(null)} aria-label="关闭">×</button><div className="reader-top"><span>{selected.category}</span><time>{selected.publishedAt}</time><i>预计阅读 {readMinutes(selected.content)} 分钟</i></div><h1>{selected.title}</h1>{selected.summary && <p className="reader-summary">{selected.summary}</p>}<div className="reader-rule"><span>正文</span></div><div className="reader-body"><MarkdownBody source={selected.content} /></div><footer className="reader-finish"><i>光</i><div><b>读到这里，感谢停留。</b><span>每一次阅读，都是让想法继续生长。</span></div><button onClick={() => setSelected(null)}>返回笔记列表</button></footer></article></div>}
  </main>;
}
