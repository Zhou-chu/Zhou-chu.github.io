...
const importedNotes = obsidianIndex as PublicNote[];
const initialNotes = [...importedNotes, ...sampleNotes];
...
export default function CleanHome() {
  ...
  useEffect(() => {
    fetch("/api/notes", { cache: "no-store" })
      ...
      .then((data) => {
        if (data?.notes?.length) setNotes([...data.notes, ...initialNotes]);
      })
      .catch(() => undefined);
  }, []);
  ...
  return <main className="clean-site">
    ...
    <section className="clean-featured">
      ...
      <div className="clean-feature-grid">{featured.map((note, index) => <a key={`${note.id}-${index}`} href={`/notes/${encodeURIComponent(note.slug)}`} style={{ "--delay": `${index * 70}ms` } as React.CSSProperties} className={index === 0 ? "primary" : ""}><div className="note-card-meta"><small>{note.category}</small><time>{note.publishedAt} · {noteMinutes(note)} 分钟</time></div><h3>{note.title}</h3><p>{note.summary}</p><span>阅读笔记 <i>↗</i></span></a>)}</div>
    </section>

    <section className="clean-layout" id="notes">
      <div className="clean-notes"><div className="clean-heading"><div><span>02</span><h2>{copy.cRecentTitle}</h2></div><p>{filtered.length} NOTES</p></div><div className="clean-list">{filtered.map((note, index) => <a key={`${note.id}-${index}`} href={`/notes/${encodeURIComponent(note.slug)}`} style={{ "--delay": `${Math.min(index, 8) * 45}ms` } as React.CSSProperties}><span className="list-index">{String(index + 1).padStart(2, "0")}</span><time>{note.publishedAt.slice(5).replace("-", ".")}</time><div><small>{note.category} · 预计 {noteMinutes(note)} 分钟{note.backlinks?.length ? ` · ${note.backlinks.length} 条反向链接` : ""}</small><h3>{note.title}</h3><p>{note.summary}</p></div><span className="list-arrow">↗</span></a>)}</div></div>
