...
summary: "+ 现在来研究均匀传输线的始端电压是角频率 $\\omega$ 的正弦时间函数时电路的稳态分析。在这种情况下，沿线各处的电压和电流也一定是同概率的正弦时间函数。这样就可以应用相量法来分析沿线的电压和电流，于是有：$$u(x,t)={\\rm Im}[\\sqrt{2}\\dot{U}(x)e^{j\\omega t}],i(x,t)={\\rm Im}[\\sqrt{2}\\dot{I}(x)e^{j\\omega t}]$$ + $Z 0=R 0+j\\"
date: 2026-07-24
category: "均匀传输线"
featured: false
---
